...
Je také důležité zmínit, že uvedený výstupní typ je tímto aliasem obalen do enumu `NodeOutput` ([@sec:enum_node_output]).
...
Enum `SoftFailPoll` slouží k zjednodušení reprezentace výstupu z asynchronní úlohy.
